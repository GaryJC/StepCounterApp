...
//        int progressWidth=sharedPrefs.getInt("progressWidth",8);
        DEFAULT_TEXT_SIZE_SP=sharedPrefs.getInt("fontSize", DEFAULT_TEXT_SIZE_SP);
        DEFAULT_STROKE_WIDTH_DP=sharedPrefs.getInt("progressWidth", DEFAULT_STROKE_WIDTH_DP);

        int red=sharedPrefs.getInt("red", 51);
        int green=sharedPrefs.getInt("green", 147);
        int blue=sharedPrefs.getInt("blue", 255);

        //int progressColor = Color.parseColor(DEFAULT_PROGRESS_COLOR);
        int progressColor = Color.rgb(red,green,blue);
        ...
        if (attrs != null) {
            ...
            dotWidth = a.getDimensionPixelSize(R.styleable.CircularProgressIndicator_dotWidth, dotWidth);
